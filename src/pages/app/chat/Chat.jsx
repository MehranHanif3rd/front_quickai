...
const Chat = () => {
  ...
  const sendMess=()=>{
    var a = document.getElementById("txtSendMessage").value;
    dispatch(
      sendMessage({
        content:a,
        sender: "me",
      ...
      <header className="border-b border-slate-100 dark:border-slate-700">
        <div className="flex py-6 md:px-6 px-3 items-center">
          <div className="flex-1">
            <div className="flex space-x-3 rtl:space-x-reverse">
              ...
              <div className="flex-1 text-start">
                ...
                <span className="block text-slate-700 dark:text-slate-300 text-xs font-normal">
                  Active now
                </span>
              </div>
            </div>
          ...
          <div className="flex-none flex md:space-x-3 space-x-1 items-center rtl:space-x-reverse">
            <div className="p-2 bg-teal-500 text-white drop-shadow-lg ">
              <Icon icon="heroicons-outline:save" />
            </div>
            <div className="p-2 bg-rose-500 text-white drop-shadow-lg">
              <Icon icon="heroicons-outline:trash" />
            </div>
            {/* Dots in Middle chat biox */}
            <div
                ...
                <div className="flex  space-x-2 items-start justify-end group w-full rtl:space-x-reverse">
                  <div className="no flex space-x-4 rtl:space-x-reverse">
                    ...
                    <div className=" whitespace-pre-wrap break-all">
                      <div className="bg-teal-500 text-contrent p-3  dark:bg-slate-900 dark:text-slate-300 text-slate-100 text-sm font-normal rounded-md flex-1 mb-1">
                      {item.content}
                      </div>
      ...
      <footer className="md:px-6 px-4 sm:flex md:space-x-4 sm:space-x-2 rtl:space-x-reverse border-t md:pt-6 pt-4 border-slate-100 dark:border-slate-700">
        ...
        <div className="flex-1 relative flex space-x-3 rtl:space-x-reverse">
          ...
          <div className="flex-none sm:flex hidden md:space-x-3 space-x-1 rtl:space-x-reverse">
            ...
            <div className="h-8 w-8 cursor-pointer bg-slate-100 dark:bg-slate-900 dark:text-teal-400 text-teal-400  flex flex-col justify-center items-center text-xl rounded-full">
              <Icon icon="heroicons-outline:microphone" />
            </div>
        </div>
          {/* Sending Message */}
          <div className="flex-none md:pr-0 pr-3 px-2 drop-shadow-lg">
            <button
              type="button"
              className="bg-teal-500 h-8 w-16 text-white flex flex-col justify-center items-center text-lg rounded-none"
              onClick={() => sendMess() }
            >
              Send
              {/* <Icon
